...
def _make_tool_call_start_packet(tool_name: str, tool_args: dict) -> str:
    """工具调用开始：前端展示'正在调用 xxx...'+ 呼吸灯"""
    return "data: " + json.dumps(
        {"type": "tool_call_start", "tool_name": tool_name, "tool_args": tool_args},
        ensure_ascii=False
    ) + "\n\n"


def _make_tool_call_done_packet(tool_name: str, result_preview: str) -> str:
    """工具调用完成：前端更新状态为已完成 + 结果摘要"""
    return "data: " + json.dumps(
        {"type": "tool_call_done", "tool_name": tool_name, "result_preview": result_preview},
        ensure_ascii=False
    ) + "\n\n"
...
async def stream_chat_response(
    ...
    try:
        ...
        yield _make_control_packet("error")
        sent_done = True
        return
